...
from eval.metrics_ir import doc_hit_at_k, mrr, ndcg_at_k, recall_at_k
...
def evaluate(retriever, queries, qrels) -> dict:
    rec, mr, nd, dh = [], [], [], []
    for q in queries:
        # Group-aware: the pieces of a split gold chunk count once (see eval.metrics_ir).
        gold = qrels[q.query_id].relevance()
        ...
        dh.append(doc_hit_at_k(gold, got, TOP_K))
    n = len(queries)
    return {
        "recall@5": sum(rec) / n,
        "mrr": sum(mr) / n,
